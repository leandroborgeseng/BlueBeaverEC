...
try {
  if (!existsSync(dir)) {
    console.log("[aion] import checklists TSE: pasta ausente — skip");
    process.exit(0);
  }
  ...
  if (expected === 0) {
    console.log("[aion] import checklists TSE: nenhum JSON — skip");
    process.exit(0);
  }
  ...
  console.log(
    `[aion] import checklists TSE: ${vinculados}/${expected} POPs SEG com procedimento`,
  );

  if (!force && vinculados >= expected) {
    console.log("[aion] import checklists TSE skipped (carga completa)");
    process.exit(0);
  }

  console.log(
    force
      ? "[aion] force — reimportando checklists TSE…"
      : `[aion] importando checklists TSE (${expected - vinculados} faltando)…`,
  );
  ...
  console.error("[aion] import checklists TSE falhou:", e);
  process.exit(1);
} finally {
