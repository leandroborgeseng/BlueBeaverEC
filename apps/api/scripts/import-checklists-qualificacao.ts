/**
 * Importa checklists de qualificação (JSON) como ProcedimentoLaudo e vincula ao Pop QLF.
 *
 * Uso:
 *   pnpm exec tsx scripts/import-checklists-qualificacao.ts
 *   pnpm exec tsx scripts/import-checklists-qualificacao.ts scripts/dados/checklists-qualificacao --force
 */
...
async function main() {
  ...
  const dir = resolve(dirArg ?? "scripts/dados/checklists-qualificacao");
    ...
    try {
      ...
      if (procId) {
        ...
        if (
          existing &&
          !force &&
          existing.tipo === TipoLaudo.QUALIFICACAO &&
