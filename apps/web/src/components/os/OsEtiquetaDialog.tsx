...
import { Overlay, WinForm, ORANGE, fld } from "./os-win-ui";
...
export function OsEtiquetaDialog({
  ...
  const osNum = osCodigo.replace(/^OS-/i, "");
  ...
  useEffect(() => {
    ...
    setData(null);
    if (!tag || tag === "—") {
      setErro("Vincule um equipamento na OS para imprimir a etiqueta.");
      return;
    }
    setBusy(true);
  ...
  function imprimir() {
    ...
    w.document.open();
    w.document.write(montarHtmlEtiqueta(data, osNum, n));
    w.document.close();
    w.focus();
    ...
    <Overlay onClose={onClose}>
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 12, fontSize: 12 }}>
          <span style={{ width: 72, textAlign: "right" }}>Nº OS</span>
        ...
        {data && (
          <div style={preview}>
            ...
            <div style={{ fontSize: 11, marginTop: 6, color: "#666" }}>OS {osNum} · QR autenticado (exige login)</div>
          </div>
        )}
      </WinForm>
    </Overlay>
...
function montarHtmlEtiqueta(data: Etiqueta, osNum: string, copias: number) {
  const bloco = `
    <div class="label">
...
<head>
  ...
  <style>
    ...
    .meta, .os { font-size: 8pt; margin-top: 2mm; }
    .os { color: #333; }
  </style>
</head>
<body>
